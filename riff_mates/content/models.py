    ...
    def clean(self):
        if self.seeking == MusicianBandChoice.MUSICIAN:
            ...
            if self.musician is not None:
                raise ValidationError("Musician field should be empty for a band seeking a musician")
        else:
            # validate seeking band
            if self.musician is None:
                raise ValidationError("Musician field required when seeking a band")
            if self.band is not None:
                raise ValidationError("Band field should be empty for a musician seeking a band")
        super.clean()
